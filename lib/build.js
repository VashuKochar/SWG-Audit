...
function renderCards(items) {
  return items.map(item => {
    let badge = '';
    if (item.difficulty) {
      badge = `<span class="level-badge level-badge-${item.difficulty}">${item.difficulty}</span>`;
    }
    let duration = '';
    if (item.duration) {
...
function buildSimulations(partials) {
  ...
  for (const cat of categories) {
    ...
    for (const levelFile of levelFiles) {
      ...
      const levelBadge = currentLevel && currentLevel.difficulty
        ? `<span class="level-badge level-badge-${currentLevel.difficulty}">${currentLevel.difficulty}</span>`
        : '';
      
      // Generate duration badge
      const durationBadge = currentLevel && currentLevel.duration
        ? `<span class="duration-badge">⏱️ ${currentLevel.duration}</span>`
        : '';
